...
{
    // 线程池
    m_pool = new ThreadPool<ChatConn>(m_actormodel, m_connPool, m_thread_num);
}
...
void Server::adjust_timer(UtilTimer *timer)
{
    time_t cur = time(NULL);
    timer->expire = cur + 3 * TIMESLOT;
...
// 关闭连接 
void Server::deal_timer(UtilTimer *timer, int sockfd)
{
    //这个函数内部要根据相应的回调函数进行错误处理
    if(users[sockfd].log_step == 3) 
    {
        users[sockfd].logout();
    }
...
bool Server::dealclientdata()
{
    ...
    if (0 == m_LISTENTrigmode)
    {
        ...
        if (ChatConn::m_user_count >= MAX_FD)
        {
